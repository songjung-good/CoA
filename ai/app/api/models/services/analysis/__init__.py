...
class AnalysisService(Generic[R], metaclass=ABCMeta):
    ...
    async def analyze(self, request: R) -> None:
        ...
        try:
            # 레포 요청 가능 여부 확인하기
            self.__update_status(dto, AnalysisStatus.CHECKING_FOR_REQUESTING)
            await self.assert_content_loadable(request)
            await self.assert_commits_loadable(request)

            # 레포에서 내용 가져오기
            self.__update_status(dto, AnalysisStatus.REQUESTING_CONTENT)
            content = await self.load_content(request)

            # 레포에서 데이터 가져오기
            self.__update_status(dto, AnalysisStatus.REQUESTING_COMMITS)
            ...
            self.__update_status(dto, AnalysisStatus.SCORING_COMMITS)
            # TODO ...
            ...
            dto.percentage = 0
            self.__update_status(dto, ex.status)

        except Exception:   # 알 수 없는 내부 오류 발생!
            dto.percentage = 0
            self.__update_status(dto, AnalysisStatus.INTERNAL_SERVER_ERROR)

    def __update_status(self, dto: AnalysisDataDto, next_status: AnalysisStatus) -> None:
        """
        Redis에 분석 상태를 업데이트합니다.
        """
        dto.status = next_status
        dto.percentage = AnalysisStatus.PERCENTAGES[dto.status]
